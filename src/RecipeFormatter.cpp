...
QString RecipeFormatter::wrapText( const QString &text, int wrapLength )
{
   ...
   QStringList line = text.split(QRegExp("\n|\r\n|\r"));
   QString wrappedText("");

   nbLines = line.size();
   for (i = 0; i < nbLines; ++i)
   {
       QString tmpStr = line.at(i);
       ...
       {
          int tempPos = pos;
          while(tmpStr.at(tempPos) != ' ' && tempPos > 0)
             tempPos--;
          if (tmpStr.at(tempPos) == ' ') //String without whitespace won't be splited
          {
             tmpStr.replace(tempPos, 1, '\n');
             pos += tempPos;
          }
          else //If the first part of the string doesn't contain a whitspace, look for the next one to split the line
          {
             tempPos = pos;
             while(tmpStr.at(tempPos) != ' ' && tempPos < len-1)
                tempPos++;
             if (tmpStr.at(tempPos) == ' ')
             {
                tmpStr.replace(tempPos, 1, '\n');
             }
             pos = pos - tempPos;
